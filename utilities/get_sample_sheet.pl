...
####################EXAMPLE OF COMMAND
 #perl /pico/work/TELET_TIGEM/VarGenius/utilities/get_sample_sheet.pl 
 # -d /gss/gss_work/DRES_TIGEM/ngsworkspace/input/BanfiLab/RPGR_project_201806/
 # -ml -fl RUN_LISTS/sb_mendelian_201810.list -t ClearSeq_inherited_diseases.bed 
 # -uid 147 -k -o /pico/work/TELET_TIGEM/ss_VarGenius/ss_sb_mendelian_201810.tsv -m exome -infreq 1
...
if ( $mode eq ''){
		die  "mode is needed [exome|targeted|rna|wgs]. Please use --mode\n";
}
...
=head2 write_sample_sheet

 Title  : write_sample_sheet
 Usage  : write_sample_sheet( - dirname => 'the folder to check',
                      );
...
sub write_sample_sheet{
	...
	if ($mode eq "targeted"){
		...
		$final_info = "rna".$sep.$perchrom.$sep.$convscores.$sep.$infreq;
		unless ( $target_bed ne "" ) {$target_bed = $rna_target;}
	}elsif ($mode eq "wgs"){#RNA
		$final_info = "wgs".$sep.$perchrom.$sep.$convscores.$sep.$infreq;
		unless ( $target_bed ne "" ) {$target_bed = $rna_target;}
	}else{
			die "Need a mode to insert the parameters! [exome, motor, exome_gr, rna]\n";
	}
...
sub parse_command_line_args{
  ...
  "\t-r2|--r2_suff: set this parameter to set how to find the fastq read 2. Default value is _R2_\n".	
	...
	" (e.g. lane1,lane2,lane3,lane4). Default value is _L001_,_L002_,_L003_,_L004_,_L005_,_L007_,_L008_\n".
  ...
  "\t-aan|--add_to_analysisname: string to add to the analysis name. Default: both the sample name and analysis name are the sample folder name. \n".
  "\t-can|--change_analysisname: set and assign same analysis name to each file of the sample sheet.\n".
