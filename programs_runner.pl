...
use LIB::programs_management qw(configFile2Hash correct_type try_exec_command
				...
				good_reads_quality  JOB_get_jobid_form_jobname alter_job execute_jobs
				...
				compress_data_to_store update_dependencies_hash save_analyses
				JOB_get_dependencies);
...
use LIB::gatk_management qw( run_GATK_RealignerTargetCreator run_GATK_IndelRealigner 
            ...
            run_GATK_GenotypeGVCFs_parallel run_GATK_HaplotypeCaller_parallel);
...
use LIB::output_management qw( import_vcf_2_db import_ann_out_2_db get_non_covered_regions
																...
																get_gene_4_interval get_non_cov_exons);
...
if ($task eq 'MS'){
	...
	if ( $pipeline eq 'pipeline'){
		...
		#When the execution is per-chromosome, hence parallelized, the samples will be merged together after PrintReads
		if ( $sequencingtype eq 'targeted' or $perchrom){
			#print_and_log( "Running the $task for the sample $sample_id...\n",$log_file);#DEBUGCODE
...
if ($task eq 'varfilt'){
  ...
  $dbh->disconnect();
	...
	if ( ($sequencingtype eq 'exome' or $sequencingtype eq 'wgs') and ($num_samples >= $cfg_hash->{'min_samples_4_joint'}) ){
...
if ($task eq 'phasing'){
	#print_and_log( "Running the $task for the analysis $sample_id...\n",$log_file);#DEBUGCODE
...
if ($task eq 'anstats'){
	#print_and_log( "Running the $task for the analysis $analysis_id...\n",$log_file);#DEBUGCODE
...
if ($task eq 'VI'){
	...
	print_and_log( "Running the $task ...\n",$log_file);#DEBUGCODE
	run_import_2_db_steps($config_file,$fold_suff);
}
...
sub run_freebayes_pipeline{
	...
	if ($cfg_hash->{'fb_varcall'} eq 'YES'){
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Getting the BAM files to be used for the calling\n",$log_file);
		foreach my $sample_id (keys %{$samples}){
			...
			if ( $multiple == 1){
				...
				#print_and_log("[an:$analysis_id ](".scalar(localtime).") There are multiple readfiles: $main_name - ".@$steps_arr1_in_use[0]."\n",$log_file);#DEBUGCODE
			}else{
				...
				#print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] There are not multiple readfiles: $main_name - ".@$steps_arr1_in_use[0]."\n",$log_file);#DEBUGCODE
			}
			...
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Using input file: $bam_input\n",$log_file);#DEBUGCODE
			if ( -e $bam_input) {
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Adding Read Group to $bam_input into $bam_temp\n",$log_file);#DEBUGCODE
				run_PICARD_AddOrReplaceReadGroups_gen($cfg_hash,$prog_used,$sample_id,$analysis_id,$bam_input,$bam_temp,$log_file);	
				move($bam_temp, $bam_input) or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot move $bam_temp in $bam_input\n";
				#Create index for bam file
				print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Indexing $bam_input into $bam_temp\n",$log_file);#DEBUGCODE	
				run_SAMTOOLS_Index_gen($cfg_hash,$bam_input,$log_file);
				
				push(@paths_to_write,$bam_input);
			}else{
				die "[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] ERROR: Cannot find file $bam_input that is needed for $task. Please run alignment step..\n";
			}
		}		
		...
		###################VARIANT CALLING
		print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Running FreeBayes using these BAM files: $bam_inputs\n",$log_file);
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Running $prog_used using $vcfFile\n",$log_file);
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline] Executing  $prog_used for filtering VCF file $sortvcf\n",$log_file);
		...
		if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
			print_and_log("[an:$analysis_id ](".scalar(localtime).") [FREEBAYES-pipeline]  Removing the following temporary files: $sortvcf..\n",$log_file);	
			delete_file($sortvcf);	
		}
	}
...
#sub run_samtools_vc_pipeline{
	...
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") [SAMTOOLS-pipeline] Getting the BAM files to be used for the calling...\n",$log_file);
	...
	#foreach my $sample_id (keys %{$samples}){
		...
		#}else{
			...
			#getSampleConfiguration_locked(\$params,$cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
								#$cfg_hash->{'db_pass'},$cfg_hash->{'db_readf_table'},$cfg_hash->{'db_readf_id'},$readfid);	
			#$main_name = $cfg_hash->{'db_readf_name'};		
			##print_and_log("[an:$analysis_id ](".scalar(localtime).") There are not multiple readfiles: $main_name - ".@$steps_arr1_in_use[0]."\n",$log_file);#DEBUGCODE
		#}
				
		#my $bam_input = $inFolder."/".get_output_name_from_executed($params,$cfg_hash->{'sort_idx_step'},
					#$params->{$main_name},$steps_arr1_in_use).".".$cfg_hash->{'bam_ext'};		
							
		##print_and_log("[an:$analysis_id ](".scalar(localtime).") Using input file: $bam_input\n",$log_file);#DEBUGCODE
		#if ( -e $bam_input) {
			#push(@paths_to_write,$bam_input);
		#}else{
			#die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot find file $bam_input that is needed for $task. Please run alignment step..\n";
		#}
	#}		
			
	#my $bam_inputs = join(" ",@paths_to_write);
	##print_and_log("[an:$analysis_id ](".scalar(localtime).") [SAMTOOLS-pipeline] ...DONE!: $bam_inputs\n",$log_file);#DEBUGCODE

	##RUN Samtools MPileup
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") [SAMTOOLS-pipeline] Executing Samtools variant calling using BAM files: $bam_inputs \n",$log_file);
	...
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") [SAMTOOLS-pipeline] Executing bcftools call\n",$log_file);
	#my $parameters = " -vmO z ";
	#my $bcf_output = "$outFolder/$analysis_name\_bcfcall.bcf";
	#run_BCFTOOLS_call($cfg_hash,$bcf_input,$bcf_output,$parameters,$log_file);
	
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") [SAMTOOLS-pipeline] Executing bcftools filter\n",$log_file);
	#$parameters = "  ";
	#my $filters = "8";
...
sub fill_steps_arrays{
	...
	if ($sequencingtype eq 'exome' or $sequencingtype eq 'targeted' or $sequencingtype eq 'wgs' ){
		$pipe_ind = 1;
	}
	if ($sequencingtype eq 'rna' ) {
		$pipe_ind = 2;
...
sub run_quality_check_steps{
	...
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting information for read file id $readf_id from database ".$cfg_hash-> {'db_name'}."...\n",$log_file);#DEBUGCODE
	...
	if ($cfg_hash->{'qc_exec'} eq 'YES' ){
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
		...
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for read file $readf_id in ".$cfg_hash-> {'db_name'}." $step=1 ...\n",$log_file);#DEBUGCODE
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Finished\t$prog_used\tread file\t$readf_id\t$partDuration\tseconds\n",$log_file);
		$partTime = time;		
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Using $qc_fq1_arch and $qc_fq2_arch to check if trimming is needed from the quality of reads.... \n",$log_file);
		if ( good_reads_quality($cfg_hash,$qc_fq1_arch,$qc_fq2_arch,$cfg_hash->{$analysis_id.'_qc_out_f'},$log_file) == 1 ){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") trimming is not needed for this read file.... \n",$log_file);
		}else{
			print_and_log("[an:$analysis_id ](".scalar(localtime).") trimming is needed for this read file.... \n",$log_file);
		}
			
	}else{print_and_log("[an:$analysis_id ](".scalar(localtime).") No quality check today..\n",$log_file);}

	##########FILLING OUTLIST FILE##########
	##########RAW QC files#####
	#From VarGenius 1.2 the paths to the html are not printed anymore into the WebPage because
	#we need to reduce the number of files present. Hence the file that will be provided is the ZIP file that will be 
	#stored with the keyword [STORE].
	#Additionally the paths in the outlist file must be written even though the program is not executed. So that the outlist.txt
	#can be constructed if needed.
	
	#build the names of the html output from FastQC
	my $qc_fq1 = $cfg_hash->{$analysis_id.'_qc_out_f'}."/".extract_name($params->{'fq1'},1)."_".$cfg_hash->{'fastqc_zip_ext'};
	my $qc_fq2 = $cfg_hash->{$analysis_id.'_qc_out_f'}."/".extract_name($params->{'fq2'},1)."_".$cfg_hash->{'fastqc_zip_ext'};

	#Check if both the files for read1 and 2 are present
	if ( check_presence($qc_fq1) and check_presence($qc_fq2) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $qc_fq1 and $qc_fq2 paths to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);	
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $qc_fq2 or $qc_fq2 are not present. Please check what is happen. \n",$log_file);	
	}	
...
sub run_trimming_steps{
	...
	if ($cfg_hash->{'trimming'} eq 'YES' ){
		...
		##In case of Trimmomatic
			...
			run_fastqc($config_file,$task,$analysis_id,$sample_id,$prog_used,$trimmed1,$trimmed2,$log_file);
				...
				log_and_exit("[an:$analysis_id ](".scalar(localtime).") Trimming program is not correct in user configuration file trim_prog = $prog_used. Pleas
		...
		if ($prog_used eq 'trimgalore'){
			...
			if ( check_presence($qc_trimmed1) and check_presence($qc_trimmed2) ){
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $qc_trimmed1 and $qc_trimmed2 paths to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $qc_trimmed1 or $qc_trimmed2 are not present. Please check what is happen. The next analysis will not be run\n",$log_file);	
			}
		}elsif ($prog_used eq 'trimmomatic'){
			#Sample sheet variables (These I will take from the DB)
			my $fq1 = $params->{'fqdir'}."/".$params->{'fq1'};
		...
		#Check if both the files for read1 and 2 are present
			if ( check_presence($trimmed1) and check_presence($trimmed2) ){
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $trimmed1 and $trimmed2 paths to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $trimmed1 or $trimmed2 are not present. Please check what is happen. The next analysis will not be run\n",$log_file);	
			}	
		}
	}
	...
	#VarGenius will copy the fastq files into the qc_out directory
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") No trimming today.. \n",$log_file);
		...
		if ($params->{$trim_step} == 0){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Copying $fq1 to $fq1_new.. \n",$log_file);
			copy($fq1,$fq1_new) or die "Cannot copy $fq1 in $fq1_new";
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Copying $fq2 to $fq2_new.. \n",$log_file);
...
sub run_rna_alignment_steps{
	...
	if ( $cfg_hash->{'align_mode'} eq 'gatk_BP'){
		...
		if ($cfg_hash->{'align_prog_exec'} eq 'YES' ){
			...
			my $runMode = "NONE";
			print_and_log("\n[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used - First Alignment: ",$log_file);
			...
			print_and_log("\n[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used - Indexing based on splice junction information: ",$log_file);
			...
			print_and_log("\n[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used - Alignment based on the new indexing: ",$log_file);
			$runMode = "NONE";
			...
			$param_in = "";
			$partDuration = time - $partTime;
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished $prog_used $partDuration seconds\n",$log_file);
			$partTime = time;	
		}
		...
		if ($cfg_hash->{'sam_sort_idx'} eq 'YES' ){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting PICARD to Add Read Group, sort and do the bam file \n",$log_file);
			my $prog_used = $cfg_hash->{'picard_arg_prog'};
			...
			if (file_not_present($input_file) > 0 ){ die "[an:$analysis_id ](".scalar(localtime).") Cannot proceed with $prog_used! Check: $input_file.\n";}
			...
			#update status
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for readf_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
			update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
						$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_readf_table'},
						$cfg_hash->{'db_readf_id'},$readf_id,$step);
			$partDuration = time - $partTime;
...
sub run_alignment_steps{
	...
	if ( $cfg_hash->{'align_mode'} eq 'gatk_BP'){
		...
		if ($cfg_hash->{'sam_sort_idx'} eq 'YES' ){
			...
			#PICARD Sort execution
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting PICARD Sorting\n",$log_file);
		...
		if ($cfg_hash->{'sam_sort_idx'} eq 'YES' ){
			...
			if ( $cfg_hash->{'reorder_sam'} eq 'YES'){
				...
				move($out_file, $input_file) or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot move $out_file in $input_file\n";
				move($out_file.".bai", $input_file.".bai") or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot move $out_file in $input_file\n";				
			}else{
...
sub run_indel_realignment_steps{
	...
	if ($cfg_hash->{'indel_realigner'} eq 'YES' ){
		...
		#Save status in the db by searching for the specific sample name
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for sample $sample_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);
		update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
					$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_readf_table'},
					$cfg_hash->{'db_readf_id'},$sample_id,$step);
		$partDuration = time - $partTime;
...
sub run_base_recalibration_steps{
  ...
  ##THIS SNIP OF CODE IS USEFUL FOR THE NEW VERSION OF GATK >= 3.8 WHERE INDEL REALIGNMENT IS NO MORE USEFUL
  ...
  ##per-chromosome, hence update the perchrom mode to not execute it
	...
	if ($cfg_hash->{'print_reads'} eq 'YES' ){
		...
		#Select GATK depending by the version
		if ( $cfg_hash->{'gatk_ver'} < 4){
			run_GATK_PrintReads_persample($cfg_hash,$prog_used,$sample_id,$analysis_id,$recal_bam,$log_file,$last_task,$task,\@steps_array,\@steps_array2);
		  }elsif ( $cfg_hash->{'gatk_ver'} >= 4){ 
			run_GATK4_ApplyBQSR($cfg_hash,$prog_used,$sample_id,$analysis_id,$recal_bam,$last_task,$task,\@steps_array,\@steps_array2,$log_file);
		  }

			  		
		#Save status in the for all read files for the specific sample name
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for all read files in sample $sample_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
	...
	##########FILLING OUTLIST FILE##########
	########################################		
	#Putting the path to the BAM RECALIBRATED into the outlist file
	if ( check_presence($recal_bam) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $recal_bam path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_recalibratedbam_desc'}."\t".$recal_bam;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		#Search BAI file and append it too
		my $recal_bai = extract_name($recal_bam,"noext").".".$cfg_hash->{'bai_ext'} ;
		if (check_presence($recal_bai)){
			$line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_recalbamindex_desc'}."\t".$recal_bai;
		}elsif ( check_presence($recal_bam.".".$cfg_hash->{'bai_ext'}) ){
			$line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_recalbamindex_desc'}."\t".$recal_bam.".".$cfg_hash->{'bai_ext'};
			}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: index BAI for $recal_bam is not present. Please check what is happen. \n",$log_file);
			}
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $recal_bam is not present. Please check what is happen. \n",$log_file);	
	}
	...
	#Update the analyses table with the status. 
	#At least one of the samples has finished the refinement step. Some other
	#sample could be in a previous task hence here I check what is the value of status
	my $status = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_status'}
...
sub update_sample_status{
	...
	if ( (scalar keys %{$distinct_readfs}) > 0){
		...
		foreach my $distinct_readf ( keys %{$distinct_readfs}){
			...
			log_and_exit( "[an:$analysis_id ](".scalar(localtime).") ERROR: The step $step_needed is needed for readfile ".
					$params->{$main_name}." and has not been completed (".$cfg_hash->{'db_readf_id'}.
					" = $distinct_readf)\n", $log_file) unless  ($params->{$step_needed} == 1) ;
		}
		#Save status in the db by searching for the specific sample name
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for sample $sample_id in ".$cfg_hash-> {'db_name'}." ($step_needed=1) ...\n",$log_file);#DEBUGCODE
		update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
					$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},
...
sub run_merge_samples_steps{
	...
	#COMMENTS HERE CAN BE REMOVED IF THE EXECUTION IS SAFE. IndelRealignment is no more in the protocol since Gatk3.8
	#The refinement has not been executed
	if (( 
	#$cfg_hash->{'realign_target_creator'} eq 'NO' and $cfg_hash->{'indel_realigner'} eq 'NO' and
	$cfg_hash->{'base_recalibrator'} eq 'NO' and $cfg_hash->{'print_reads'} eq 'NO' and $sequencingtype eq 'targeted' )){
				$last_step	= $cfg_hash->{'sort_idx_step'};
				$last_task	= $cfg_hash->{'align_task'};
	}
	...
	#$cfg_hash->{'realign_target_creator'} eq 'YES' and $cfg_hash->{'indel_realigner'} eq 'YES' and
			$cfg_hash->{'base_recalibrator'} eq 'YES' and $cfg_hash->{'print_reads'} eq 'YES'){
			$last_step	= $cfg_hash->{'print_reads_step'};
			$last_task	= $cfg_hash->{'refine_task'};
	}else{
		print_and_log("[an:$analysis_id ](".scalar(localtime).") ERROR: There is something wrong with your setting. This refinement is not allowed..\n",$log_file);
	}
		
	
	if ( $multiple == 1 or $perchrom == 1){
		#Here we need that the last step has been done. 
		...
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") multiple: $multiple and mergesam=".$cfg_hash->{'mergesam'} eq 'YES'."\n",$log_file);#DEBUGCODE
		#Creating targets for the realignment
		if ($cfg_hash->{'mergesam'} eq 'YES' ){
			...
			#Save status in the db by searching for the specific sample name
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating sample status in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);
			update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
					$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},
					$cfg_hash->{'db_sample_id'},$sample_id,$step,$cfg_hash->{'db_analysis_id'},$analysis_id);
			$partDuration = time - $partTime;
			...
			if ($cfg_hash->{'index_after_merge'} eq 'YES' ){
				...
				#my $numExec = 1;
					...
					#foreach my $exec (@chroms){
						...
						##print "Starting SAMTOOLS index for $input_file\n";		
						##Checks if the SAM output exists and is non-zero
						#if ( file_not_present($input_file) > 0){ log_and_exit( "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot find $input_file. Please check merge_groups task\n", $log_file)}
						#run_SAMTOOLS_Index_gen($cfg_hash,$input_file,$log_file);
					#}
				#}#Otherwise, if it has not been performed per-chromosome
				#else{
					#Get the single bam file and index it
					my $input_file = $inFolder."/".build_input_name_from_executed($params,$step,
										$sample_name,\@steps_array2)."_".$step.".".$cfg_hash->{'bam_ext'};
											
					#Checks if the SAM output exists and is non-zero
					if ( file_not_present($input_file)  > 0){ log_and_exit( "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot find $input_file. Please check $last_task task\n", $log_file)}
					run_SAMTOOLS_Index_gen($cfg_hash,$input_file,$log_file);
				#}
			}	
	...
	}else{ print_and_log( "[an:$analysis_id ](".scalar(localtime).") Step mergesam for sample $sample_id is not needed\n",$log_file);}					    
...
#=cut
#sub run_merge_samples_stepsOLD{
	#my $config_file = shift;
	...
	##Here we need the print reads step has been done. 
		...
		##INDEXING of the BAM file
			...
			#my $numExec = 1;
			#if ( $perchrom == 1 ){
				###Using also Chromosomex X and Y that
				#$numExec = $cfg_hash->{'chromosomes_num'};
				#my @chroms = 1..$numExec;
				#push(@chroms,split(",",$cfg_hash->{'sex_chromosomes'}));
...
sub run_variant_calling_steps{
	...
	#print_and_log("\n####################################################...\n",$log_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting variant calling task \n",$log_file);
	#print_and_log("####################################################...\n",$log_file);
	#Verifying that there has been an execution of multiple samples
	my $multiple = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
						    $cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'db_multiple'},
	...
	#Since Gatk3.8 IndelRealignment is no more useful. Hence this if are commented
	#The refinement has not been executed
	...
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") As from the pipeline you want to execute: last step: $last_step - last_task: $last_task - seqtype: $sequencingtype\n",$log_file);#DEBUGCODE
	
	#In only one cases we need a step of merge:
	#1: multiple read files and no refinemnet step
	if ( $multiple == 1 and $printreads ==0 ){
		$step_needed  = $cfg_hash->{'mrdup_groups_step'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Multiple samples are present. Taking the input from $last_task folder...\n",$log_file);
	}else {
		$step_needed  = $last_step;
		update_sample_status($cfg_hash,$sample_id,$step_needed);
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Multiple samples are not present. Taking the input from $last_task folder...\n",$log_file);
	}
	
	##Just to set the exetension of the VCF file per sample...
	...
	if ($dogenot){
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") The genotype step has to be run. The output extension will be: $out_ext...\n",$log_file);
	}else{
		$out_ext = $cfg_hash->{'vcf_ext'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") The genotype will not be run. The output extension will be: $out_ext...\n",$log_file);
	}
	...
	if ($cfg_hash->{'varcall'} eq 'YES' ){
		...
		#Case of no refinement step: there is a single file and no refinement step was executed: 
		#construct the name using readfile level fields
		if ( $step_needed eq $cfg_hash->{'sort_idx_step'} ){
			$steps_arr1_in_use = \@steps_array;
			my $distinct_samples = select_distinct_samples($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
									$cfg_hash->{'db_pass'},$cfg_hash->{'db_readf_table'},$cfg_hash->{'db_readf_id'},
									$cfg_hash->{'db_analysis_id'},$analysis_id,$cfg_hash->{'db_sample_id'},$sample_id);
		...
		#CASE: BR or BR-M or ALN-M: there are multiple read files or the last step is BR
		...
		#InFolder depends by the last step
		my $inFolder = $cfg_hash->{$analysis_id.'_'.$in_fold_suff.'_out_f'};
		my $outFolder = $cfg_hash->{$analysis_id.'_'.$out_fold_suff.'_out_f'};
		...
		#If	 the execution is per-chromosome (eg. for WGS)
		if ( $perchrom == 1 ){		
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			...
			#Select GATK depending by the version
			if ( $cfg_hash->{'gatk_ver'} < 4){
				($jobs_to_wait,$vcfs,$final_vcf) = run_GATK_HaplotypeCaller_parallel($cfg_hash,$prog_used,$sample_id,$analysis_id,$input_file,$out_file,$inFolder,$outFolder,$log_file);
			}elsif ( $cfg_hash->{'gatk_ver'} >= 4){ 
				#This function runs threads instead of jobs
				($jobs_to_wait,$vcfs,$final_vcf) = run_GATK4_HaplotypeCaller_parallel($cfg_hash,$prog_used,$sample_id,$analysis_id,$in_fold_suff,$out_fold_suff,$step,$step_needed,\@steps_array,\@steps_array2,$log_file);
			}
			...
			$prog_used = $cfg_hash->{'catvar_prog'};
			$step = $cfg_hash->{'catvar_step'};			
			#Launch a job to combine the VCF obtained but get the command before
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Launch a job to combine the VCFs obtained:\n ",$log_file);
			my $combine_cmd = "";

			#Select GATK depending by the version
			if ( $cfg_hash->{'gatk_ver'} < 4){
				$combine_cmd = run_GATK_CatVariants($cfg_hash,$prog_used,$sample_id,$analysis_id,$log_file,$task,$step,\@steps_array2,"GET");
			}elsif ( $cfg_hash->{'gatk_ver'} >= 4){ 
				$combine_cmd = run_GATK4_CatVariants($cfg_hash,$prog_used,$sample_id,$analysis_id,$log_file,$task,$step,\@steps_array2,"GET");#NON AGGIORNATA
			}		
	  			
			#Run a job for CatVariants
			...
			my $name = $step."_".$sample_id;
			my $combvar_job = execute_jobs($cfg_hash,$analysis_id,\@commands,$exec_job_script,$jobs_to_wait,$task,$name,$log_file);
			
			
			#Now alter the dependencies of the genotyping job: it has to wait the combine variant job
			my $job_name = $cfg_hash->{'genot_task'}."_a".$analysis_id;
			my $genot_jobid = JOB_get_jobid_form_jobname($job_name,$cfg_hash->{'qsub_username'});
			print_and_log("Searching dependencies of $job_name ($genot_jobid ):\n ",$log_file);
			my $old_depend = JOB_get_dependencies($genot_jobid,$log_file);
			
			my $new_depend = "-W depend=afterok:$combvar_job";
			if ( $old_depend !~ /finished/ ){
				$new_depend .= ",$old_depend"; 
			}
			print_and_log("DEPS: old: $old_depend - new: $new_depend  \n ",$log_file);
			#Update the hash with dependencies
			my $deps_f = $cfg_hash->{'main_data_folder'}."/".$cfg_hash->{'jc_dep_hash_f'};
			update_dependencies_hash($deps_f,$job_name,$genot_jobid,$combvar_job,$log_file);
			
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Altering dependencies of ".$cfg_hash->{'genot_task'}."_a".$analysis_id." ($genot_jobid):\n ",$log_file);
			alter_job($genot_jobid,$new_depend,$log_file);
			...
			#Select GATK depending by the version
			if ( $cfg_hash->{'gatk_ver'} < 4){
				run_GATK_HaplotypeCaller_parallel($cfg_hash,$prog_used,$sample_id,$analysis_id,$input_file,$out_file,$inFolder,$outFolder,$log_file);
			}elsif ( $cfg_hash->{'gatk_ver'} >= 4){ 
				run_GATK4_HaplotypeCaller_parallel($cfg_hash,$prog_used,$sample_id,$analysis_id,$in_fold_suff,$out_fold_suff,$step,$step_needed,\@steps_array,\@steps_array2,$log_file);
			}
		...
		##At this point VarGenius can remove all the files from different read files if samples are separated
			...
			if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
				...
				if ( $bamfilesused ==  ($cfg_hash->{'tasks_using_bam'} - 1) )	{
					foreach my $bam_path (@paths){
						print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the alignment $bam_path file..\n",$log_file);	
		...
		#print_and_log("\n####################################################...\n",$log_file);
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Variant calling task completed.\n",$log_file);
		#print_and_log("####################################################...\n",$log_file);								
		}	
	}
}	
	
=head2 run_variant_calling_stepsOLD
...
sub run_genotyping_steps{
	...
	#print_and_log("\n####################################################...\n",$log_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting genotyping task...\n",$log_file);
	#print_and_log("####################################################...\n",$log_file);
	...
	my $inFolder = $cfg_hash->{$analysis_id.'_'.$task.'_out_f'}; 
	...
	if ($cfg_hash->{'genot'} eq 'YES' ){
		...
		my $params_in;
		getSampleConfiguration_locked(\$params_in,$cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
								$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_id'},$analysis_id);			
		my $outFile = $inFolder."/".build_input_name_from_executed($params_in,$step,$analysis_name,\@steps_array3)."_$step.".$cfg_hash->{'vcf_ext'};
			
						
		#If the number of samples is huge or in any case when using GATK4
		if ( $num_samples >= $cfg_hash->{'min_samples_4_joint'} ){		
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used for JOINT analysis ($num_samples) samples: ",$log_file);
			...
			if ( $cfg_hash->{'gatk_ver'} < 4){
				...
				#ALE		
				#Version 4 of GATK needs the generation of the Genomics DB to perform the genotyping	
				#Genomics db import      
			     my $jobs_to_wait_dbi  = "no_depend";
			     if ($cfg_hash->{'gatk_run_GenomicsDBImport'} eq "YES"){
					$prog_used = $cfg_hash->{'gendb_import_prog'};
					 print_and_log( "\nRunning vcf files with $prog_used ...\n",$log_file);#DEBUGCODE

					 $jobs_to_wait_dbi = run_GATK4_GenomicsDBImport_parallel($cfg_hash,$prog_used,$analysis_id,$sample_id,$task,$step,\@steps_array2,$log_file);
					 print_and_log( "$prog_used finished ...\n",$log_file);#DEBUGCODE
				 }
				
				#Genotype GVCFs
				#If you want to make only GATHERvcfs without re-genotype
				 my $get_ret = "RUN";
				 if ($cfg_hash->{'gatk_run_GenotypeGVCFs'} ne "YES"){
					$get_ret = "GET_VCF_LIST";
				 }
				 $prog_used = $cfg_hash->{'genot_prog'};
				 print_and_log( "Running  $prog_used for GVCFs joint genotyping  ...\n",$log_file);#DEBUGCODE

				 ($jobs_to_wait,$vcfs,$final_vcf) = run_GATK4_GenotypeGVCFs_parallel($cfg_hash,$prog_used,$analysis_id,$sample_id,$task,$step,$step_needed,\@steps_array2,\@steps_array3,$jobs_to_wait_dbi,$log_file,$get_ret);
				 print_and_log( "$prog_used  finished ...\n",$log_file);#DEBUGCODE				 
			...
			#Launch a job to combine the VCF obtained but get the command before
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Launch a job to combine the VCF obtained:\n ",$log_file);
			my $combine_cmd = "";
			...
			if ( $cfg_hash->{'gatk_ver'} < 4){
			  ...
			  #For GATK4 joint genotyping is suggested to use PICARD GatherVcfs (September 2019)
			  $prog_used = $cfg_hash->{'picard_gathervcfs_prog'};
			  print_and_log( "Running $prog_used ...\n",$log_file);#DEBUGCODE
			  #Use GET_CMD to get the command and run it here in a new job that waits some job ids
			  $combine_cmd = run_PICARD_GatherVcfs($cfg_hash,$prog_used,$vcfs,$outFile,$log_file,"GET_CMD");
			}
			
			#Run a job for CombineVariants
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Run a job for CombineVariants:\n ",$log_file);
			my @commands = ($combine_cmd);
			#Path to the script to execute multiple jobs using an array of commands
			my $exec_job_script = $cfg_hash->{'lib_folder'}."/".$cfg_hash->{'exec_job_script'};
			...
			my $varfilt_jobid = JOB_get_jobid_form_jobname($job_name,$cfg_hash->{'qsub_username'});
			if ( $varfilt_jobid > 0){
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Altering dependencies of "."varfilt_a".$analysis_id." ($varfilt_jobid):\n ",$log_file);
				alter_job($varfilt_jobid,$new_depend,$log_file);

				#Update the hash with dependencies
				my $deps_f = $cfg_hash->{'main_data_folder'}."/".$cfg_hash->{'jc_dep_hash_f'};
				update_dependencies_hash($deps_f,$job_name,$varfilt_jobid,$combvar_job,$log_file);				
			}

		}else{
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used for standard analysis ($num_samples) samples : ",$log_file);
		  if ( $cfg_hash->{'gatk_ver'} < 4){
			...
			run_GATK_GenotypeGVCFs($cfg_hash,$prog_used,$sample_id,$analysis_id,$log_file,$task,$step,$step_needed,\@steps_array2,\@steps_array3);
			  ...
			  #GATK4 needs to generate the GenomicsDB 
			  if ($cfg_hash->{'gatk_run_GenomicsDBImport'} eq "YES"){
					$prog_used = $cfg_hash->{'gendb_import_prog'};
					 print_and_log( "\nRunning vcf files with $prog_used ...\n",$log_file);#DEBUGCODE

					 run_GATK4_GenomicsDBImport_parallel($cfg_hash,$prog_used,$analysis_id,$sample_id,$task,$step,\@steps_array2,$log_file);
					 print_and_log( "$prog_used finished ...\n",$log_file);#DEBUGCODE
			  }
			 #GenotypeGVCFs
			 if ($cfg_hash->{'gatk_run_GenotypeGVCFs'} ne "YES"){
				#GenotypeGVCFsparallel needs the jobs to wait prior to be run. In this case we give nothing
				my $jobs_to_wait_dbi = "";
				my $get_ret = "RUN";
				 $prog_used = $cfg_hash->{'genot_prog'};
				 print_and_log( "Running  $prog_used for GVCFs joint genotyping  ...\n",$log_file);#DEBUGCODE
				 run_GATK4_GenotypeGVCFs_parallel($cfg_hash,$prog_used,$analysis_id,$sample_id,$task,$step,$step_needed,\@steps_array2,\@steps_array3,$jobs_to_wait_dbi,$log_file,$get_ret);
				 print_and_log( "$prog_used  finished ...\n",$log_file);#DEBUGCODE			
			 }	
			...
			#Save status in the db by searching for the specific sample name
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for group $sample_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
		update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
				$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},
	...
	#print_and_log("\n####################################################...\n",$log_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Genotyping task completed.\n",$log_file);
	#print_and_log("####################################################...\n",$log_file);	
}
...
sub run_var_filt_INDEL_SNP_steps{
	...
	if ($cfg_hash->{'varfilt'} eq 'YES' ){
		if ( $db_step_needed == 1){
			...
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting information for $analysis_name from database ".$cfg_hash-> {'db_name'}."...\n",$log_file);#
			...
			#print_and_log( "[an:$analysis_id ](".scalar(localtime).") Configuration finished input: $input_file and output: $outFile \n",$log_file);#DEBUGCODE
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).")  RUNNING $prog_used for $vartype : ",$log_file);
			...
			$partDuration = time - $partTime;
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished $prog_used [$vartype] $partDuration seconds\n",$log_file);
			$partTime = time;
			...
			if ( $cfg_hash->{'gatk_ver'} < 4){
				...
				run_GATK4_VariantFiltration($cfg_hash,$prog_used,$indelsFile,$filtIndOut,$param_str,$cfg_hash->{'indel_filters'},$log_file);
			}	
			...
			if ( $cfg_hash->{'gatk_ver'} < 4){
				...
				if (defined $cfg_hash->{'VF_clusterWindowSize'}){$param_str .= " --cluster-window-size ".$cfg_hash->{'VF_clusterWindowSize'}." ";}
				if (defined $cfg_hash->{'VF_clusterSize'}){$param_str .= " --cluster-size ".$cfg_hash->{'VF_clusterSize'}." ";}							
				run_GATK4_VariantFiltration($cfg_hash,$prog_used,$snpsFile,$filtSnpOut,$param_str,$cfg_hash->{'snp_filters'},$log_file);
			}	
			...
			#Save status in the db by searching for the specific sample name
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for analysis $analysis_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
			update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
					$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},
					$cfg_hash->{'db_analysis_id'},$analysis_id,$step);
			...
			if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the following temporary files: $snpsFile\n$indelsFile\n$filtSnpOut\n$filtIndOut..\n",$log_file);	
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $step_needed has not been run. Execute this before varfilt step.. \n",$log_file);
		}
	}
...
sub run_variant_quality_score_recalibration_steps{
	...
	#(here sample_id is the analysis name, since we bring this value from vargenius.pl)
	my $analysis_name = $sample_id;
	...
	if ($dogenot){
		...
		#Checks that step_needed is executed for all the samples and updates that step
		#in the group table
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for analysis $analysis_id in ".$cfg_hash-> {'db_name'}." ($step_needed=1) ...\n",$log_file);	#DEBUGCODE
		update_group_status($cfg_hash,$analysis_id,$step_needed);
	}
	...
	getSampleConfiguration_locked(\$params_in,$cfg_hash->{'db_name'},$cfg_hash-> {'db_dsn'},
						$cfg_hash-> {'db_user'},$cfg_hash-> {'db_pass'},$table_4_name,$id_4_name,$sample_or_gr_id);
	#Setting the input file name (could be gvcf or vcf)
	my $input_file = $inFolder."/".build_input_name_from_executed($params_in,$step,$analysis_name,\@steps_arr).".$extension ";
			
	#Set the unique output name (always VCF)
	my $params_out;
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting information for ".$cfg_hash->{'db_analysis_id'}.":$analysis_id from database ".$cfg_hash-> {'db_name'}."...\n",$log_file);#DEBUGCODE
	getSampleConfiguration_locked(\$params_out,$cfg_hash-> {'db_name'},$cfg_hash-> {'db_dsn'},
						$cfg_hash-> {'db_user'},$cfg_hash-> {'db_pass'},$cfg_hash->{'db_analyses_table'},$cfg_hash->{'db_analysis_id'},$analysis_id);
	...
	if ( $var_num < 100000){
		print_and_log("[an:$analysis_id ](".scalar(localtime).") WARNING: VCF file $input_file has $var_num variants. (Consider that VQSR could stop with less than 100K)\n",$log_file);
	}
	...
	if ( ($cfg_hash->{'varrecal'} eq 'YES')  ){	
		...
		#SNP Output files
		my $mode = "SNP";
		my $recal_file_snp = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.recal";
		my $tranches_file_snp = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.tranches";
		my $rscript_file_snp = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.r";				
		#Setting the first output file name (will be a vcf with only SNPs recalibrated and raw INDELs)
		my $recal_snps_raw_indels = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_$step\_$mode.".$cfg_hash->{'vcf_ext'};
		my $recal_snps_and_indels = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_$step.".$cfg_hash->{'vcf_ext'};
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Doing VQSR for $mode...\n",$log_file);#DEBUGCODE
		#Select GATK depending by the version
		if ( $cfg_hash->{'gatk_ver'} < 4){
		 $prog_used = $cfg_hash->{'varrecal_prog'};
			...
			run_GATK_ApplyRecalibration($cfg_hash,$prog_used,$sample_id,$analysis_id,$mode,$input_file,$recal_file_snp,$tranches_file_snp,$rscript_file_snp,$recal_snps_raw_indels,$log_file);
			 ...
			 $prog_used = $cfg_hash->{'varrecal_prog'};
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			run_GATK4_VariantRecalibration($cfg_hash,$prog_used,$mode,$input_file,$recal_file_snp,$tranches_file_snp,$rscript_file_snp,$log_file);

			$prog_used = $cfg_hash->{'apprecal_prog'};	
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			run_GATK4_ApplyVQSR($cfg_hash,$prog_used,$mode,$input_file,$recal_file_snp,$tranches_file_snp,$rscript_file_snp,$recal_snps_raw_indels,$log_file);
		}
					

		#INDEL Execution (uses as input the first output obtained with SNPs)
		$mode = "INDEL";
		my $recal_file_indel = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.recal";
		my $tranches_file_indel = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.tranches";
		my $rscript_file_indel = $outFolder."/".build_input_name_from_executed($params_out,$step,$analysis_name,\@steps_array3)."_".$step."_$mode.r";				
		...
		if ( $cfg_hash->{'gatk_ver'} < 4){		
			...
			$prog_used = $cfg_hash->{'varrecal_prog'};				
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			run_GATK4_VariantRecalibration($cfg_hash,$prog_used,$mode,$recal_snps_raw_indels,$recal_file_indel,$tranches_file_indel,$rscript_file_indel,$log_file);

			$prog_used = $cfg_hash->{'apprecal_prog'};	
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			run_GATK4_ApplyVQSR($cfg_hash,$prog_used,$mode,$recal_snps_raw_indels,$recal_file_indel,$tranches_file_indel,$rscript_file_indel,$recal_snps_and_indels,$log_file);

		}
		
		#Save status in the db 
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for group $analysis_id in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
		update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
				$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},
				$cfg_hash->{'db_analysis_id'},$analysis_id,$step);
		$partDuration = time - $partTime;
...
sub run_genotype_refinement_steps{
	...
	if ($cfg_hash->{'genotref'} eq 'YES' ){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") STARTING GENOTYPE REFINEMENT... \n",$log_file);
			##Getting the pedigree file
			
			my $all_ped_file = $cfg_hash->{$analysis_id."_data_fold"}."/$analysis_name.".$cfg_hash->{'ped_ext'};	
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Obtaining a complete PED file : $all_ped_file \n",$log_file);
		...
		if ( $cfg_hash->{'gatk_ver'} < 4){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			my $vcf_posteriors = 	$input_file."_pos".".".$cfg_hash->{'vcf_ext'};
			#my $param_str = " --supporting ".$cfg_hash->{'gatk_ref_f'}."/".$cfg_hash->{$cfg_hash->{'GRW_CGP_supporting'}}." ";	#Used with GATK3.8
			my $param_str = "";	
			run_GATK4_CalculateGenotypePosteriors($cfg_hash,$prog_used,$input_file,$all_ped_file,$vcf_posteriors,$param_str,$log_file);
			...
			#Set the filters by searching in the user file
			$param_str = " ";			
			run_GATK4_VariantFiltration($cfg_hash,$prog_used,$vcf_posteriors,$vcf_gen_filtered,$param_str,$cfg_hash->{'GRW_genotype_filters'},$log_file);
		
			$prog_used = $cfg_hash->{'varann_prog'};	
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
			my $vcf_gfilt_denovo = 	$input_file."_gFiltered_denovo.".$cfg_hash->{'vcf_ext'};	
			run_GATK4_VariantAnnotator($cfg_hash,$prog_used,$input_file,$out_vcf,$all_ped_file,$log_file);
...
sub run_phasing_steps{
	...
	my $input_file = $inFolder."/".build_input_name_from_executed($params,$step,$analysis_name,\@steps_array3).".".$cfg_hash->{'vcf_ext'};
	#Set the unique output name
	my $out_vcf = $outFolder."/".build_input_name_from_executed($params,$step,$analysis_name,\@steps_array3)."_$cfg_hash->{'phasing_step'}.".$cfg_hash->{'vcf_ext'};
	...
	if ($cfg_hash->{'phasing'} eq 'YES' ){
		...
		if ( $filter_step == 1 ){
			...
			my $ped_file = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
										...
										$cfg_hash->{'db_analysis_id'},$analysis_id);	
			if ( $ped_file eq 'none' ){ 
				log_and_exit("[an:$analysis_id ](".scalar(localtime).") WARNING: the analysis $analysis_id does not have a pedigree file. $prog_used will not be run\n",$log_file);
			}
			...
			#Save status in the db by searching for the specific sample name
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating analysis status for analysis $analysis_name in ".$cfg_hash-> {'db_name'}." ($step=1) ...\n",$log_file);#DEBUGCODE
			update_analysis_status_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
					$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_analyses_table'},
					$cfg_hash->{'db_analysis_id'},$analysis_id,$step);
			$partDuration = time - $partTime;
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished\t$prog_used\tanalysis\t$analysis_name\t$partDuration\tseconds\n",$log_file);
			$partTime = time;		
		
		
		}else{
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot run ".$cfg_hash->{'phasing_step'}." step because $step_needed has not been run...\n",$log_file);
		}
	}
	...
	if ( -e $mendel_viols_out){
		...
		if ( check_presence($mendel_viols_out) ){
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $mendel_viols_out path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
			my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_mendviol'}."\t".$mendel_viols_out;
			append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		}else{
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $mendel_viols_out is not present. Please check what is happen. \n",$log_file);	
		}		
		########################################
		...
		if ( !-z $mendel_viols_out){
			...
			if (defined $num_mend_viol){
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending number of mendelian violation to ".$cfg_hash->{$analysis_id."_outstats_file"}." file... \n",$log_file);
...
sub run_coverage_steps{
	...
	#Get the parameters from input
	my $cfg_hash = load_hash($config_file);
	print_and_log("\n[an:$analysis_id ](".scalar(localtime).") Starting alignment statistics with Flagstat and non-covered region analysis [per-sample]..\n",$log_file);
	...
	if ( $multiple == 1){
		print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Sample $sample_name is sub-divided in multiple read files. A merge is needed..\n",$log_file);
		...
		print_and_log("\n[an:$analysis_id ](".scalar(localtime).") Input BAM (eventually duplicates removed): $bam_input - BAM aligned only: $bam_aligned..\n",$log_file);#DEBUGCODE
		
		#If readfiles are multiple and one among the flagstat and consensus approach step
		#must be executed, they must be merged
		if ($cfg_hash->{'flagstat_mrdup'} eq 'YES' 	or $cfg_hash->{'consensus_vc_approach'} ne 'NO' ){
			#If the merged BAM file has not been already produced or size is zero			
			 if ( !-e $bam_input or -z  $bam_input){
					if ( $cfg_hash->{'mergebam'} eq 'YES' ){
						...
						print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Starting MergeBam to merge multiple samples..\n",$log_file);
						...
						print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING Sort : ",$log_file);
						run_SAMTOOLS_Sort_gen($cfg_hash,$bam_input_merged,$bam_input_merge_sort,$log_file);
						print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING Index : ",$log_file);
						...
						#The variant calling step, for example, uses them. 
						if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
							print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Removing read files\n",$log_file);
							#Check if this is the last task using the BAM files 
							my $bamfilesused = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
												$cfg_hash->{'db_pass'},$cfg_hash->{'db_sample_table'},$cfg_hash->{'db_sample_bamfilesused'},
												$cfg_hash->{'db_sample_id'},$sample_id);	
							if ( $bamfilesused == ($cfg_hash->{'tasks_using_bam'} - 1) )	{
								...
								print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Read files will not be removed. Still ".($cfg_hash->{'tasks_using_bam'} - 1) - $bamfilesused." steps have to use them..\n",$log_file);
							}
						}							
						#Print time needed
						$partDuration = time - $partTime;
		...
		if ($cfg_hash->{'flagstat_sorted'} eq 'YES' and $cfg_hash->{'mergebam'} eq 'YES'){
			 ...
			 if ( !-e $bam_aligned ){
				print_and_log("\n[an:$analysis_id ](".scalar(localtime).") Merging all the SAM files in a unique BAM..\n",$log_file);
				my $main_name = $cfg_hash->{'db_readf_name'};
				...
				my $samples_to_merge  = " I=".join(" I=",@paths)." ";
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Samples: $samples_to_merge..\n",$log_file);
				my $prog_used = $cfg_hash->{'mergesam_prog'};	
				print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used : ",$log_file);
				...
				if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
					foreach my $path (@paths){
						print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the alignment $path file..\n",$log_file);	
						delete_file($path);						
					}
				}
			}	
		...
		print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") There are no multiple samples..\n",$log_file);
		my $params;
		my $main_name = $cfg_hash->{'db_readf_name'};
		getSampleConfiguration_locked(\$params,$cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
		...
		$bam_input = $inFolder."/".get_output_name_from_executed($params,$cfg_hash->{'mark_rem_dup_step'},
								$params->{$cfg_hash->{'db_readf_name'}},\@steps_array).".".$cfg_hash->{'bam_ext'};
		
		$bam_aligned = $inFolder."/".get_output_name_from_executed($params,$cfg_hash->{'sort_idx_step'},
								$params->{$cfg_hash->{'db_readf_name'}},\@steps_array).".".$cfg_hash->{'bam_ext'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") bam input: $bam_input\n",$log_file);
		...
		if ($cfg_hash->{'flagstat_sorted'} eq 'YES' and $cfg_hash->{'mergebam'} eq 'YES'){
			if (! (-e $bam_aligned) or -z  $bam_aligned ){
				#The SAM file is produced just after the alignment hence as suffix needs only those before sortidx
				my $sam_aligned =  $inFolder."/".build_input_name_from_executed($params,$cfg_hash->{'sort_idx_step'},
									$params->{$cfg_hash->{'db_readf_name'}},\@steps_array).".".$cfg_hash->{'sam_ext'};
				if (-e $sam_aligned){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING  View : ",$log_file);
					...
					if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
						...
						log_and_exit("[an:$analysis_id ](".scalar(localtime).") File $sam_aligned does not exists. Please run the alignment...\n",$log_file);
				}
			}	
		}	
	}
	
	#If there was an execution per chromosome then also the base recalibrated files must be merged for statistics
	if ($perchrom == 1){
		my $prog_used = $cfg_hash->{'mergebam_prog'};
		#Here get the samples to be merged
		my @paths = ();
		...
		foreach my $exec (@chroms){		
			my $ith_input = extract_name($bam_recalibrated,'noext')."_".$exec.".".$cfg_hash->{'bam_ext'};
			if (file_not_present($ith_input) > 0 ){ die "Cannot proceed with $prog_used! Check: $ith_input.\n";}
			push(@paths,$ith_input);
		}

		print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Starting MergeBam to merge Base Recalibrated files per chromosome...\n",$log_file);

		my $samples_to_merge  = " I=".join(" I=",@paths)." ";
		#Create the name for the bam file of the merge from the lane files
		my $recalibrated_merged = extract_name($bam_recalibrated,'noext')."_".$cfg_hash->{'mergebam_step'}.".".$cfg_hash->{'bam_ext'};						
		
		print_and_log( "RUNNING $prog_used : ",$log_file);#DEBUGCODE
		run_PICARD_MergeSamFiles_gen($cfg_hash,$prog_used,$samples_to_merge,$recalibrated_merged,$log_file);
		
		$bam_recalibrated = $inFolder."/".extract_name($recalibrated_merged,1)."_".$cfg_hash->{'sort_idx_step'}.".".$cfg_hash->{'bam_ext'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING Sort : ",$log_file);
		run_SAMTOOLS_Sort_gen($cfg_hash,$recalibrated_merged,$bam_recalibrated,$log_file);
		print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING Index : ",$log_file);
		run_SAMTOOLS_Index_gen($cfg_hash,$bam_recalibrated,$log_file);		
	}
	...
	my $noncovexons_out = extract_name($bam_recalibrated,"noext")."_exons".$cfg_hash->{'noncov_ext'};
	if ($cfg_hash->{'noncovered_regions'} eq 'YES' ){
			#METHOD WITH GENOMECOV. IT IS EXECUTED AUTOMATICALLY
			if ( -e $bam_recalibrated and !(-z  $bam_recalibrated) ){
				print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") Starting the step to get $task_name : ",$log_file);
				#Get non covered regions
				#get_non_cov_reg_with_bedtools($cfg_hash,$bam_recalibrated,$noncovgenes_out,$analysis_id,$log_file);
				
				#Get non covered exons
				get_non_cov_exons($cfg_hash,$bam_recalibrated,$noncovexons_out,$analysis_id,$log_file);
				
				#print_and_log( "\n[an:$analysis_id ](".scalar(localtime).") The output file is: $noncovgenes_out..\n",$log_file);#DEBUGCODE
				$partDuration = time - $partTime;
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished get $task_name $partDuration seconds\n",$log_file);
				$partTime = time;		
			}else{print_and_log( "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot get $task_name for sample $sample_id. Input file $bam_recalibrated does not exists.\n",$log_file);}
	}
	...
	#	print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $bam_input path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
	#	my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_sortedbam_desc'}."\t".$bam_input;
	#	append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
	#}else{
	#	print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $bam_input is not present. Please check what is happen. \n",$log_file);	
	#}		
	
	#Put the path to the non covered regions file in the outlist file
	if ( check_presence($noncovgenes_out) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $noncovgenes_out path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_final_noncov'}."\t".$noncovgenes_out;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $noncovgenes_out is not present. Please check what is happen. \n",$log_file);	
	}		
	...
	if ($cfg_hash->{'flagstat_sorted'} eq 'YES' ){
		...
		if ( check_presence($flagstat_aln) ){
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $flagstat_aln path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
			my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_FS_alignsort_desc'}."\t".$flagstat_aln;
			append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		}else{
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $flagstat_aln is not present. Please check what is happen. \n",$log_file);	
		}		

		#Remove the SAM files of the aligned and sorted used for the statistics
		if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the alignment $bam_aligned file..\n",$log_file);	
			delete_file($bam_aligned);						
		}	
		
		$partDuration = time - $partTime;
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished $prog_used (flagstat_sorted) $partDuration seconds\n",$log_file);
		$partTime = time;		
	...
	if ($cfg_hash->{'flagstat_baserecal'} eq 'YES' ){
			...
			if (-e $bam_recalibrated){ 
				...
				my $prog_used = $cfg_hash->{'flagstat_prog'};	
				print_and_log("[an:$analysis_id ](".scalar(localtime).") SAMTOOLS_flagstat execution using to get statistics for the file with removed duplicates..\n",$log_file);	
				...
				if ( check_presence($flagstat_recalibrated) ){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Appending $flagstat_recalibrated path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
					my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_FS_remdup_desc'}."\t".$flagstat_recalibrated;
					append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
				}else{
					print_and_log("[an:$analysis_id ](".scalar(localtime).") WARNING: $flagstat_recalibrated is not present. Please check what is happen. \n",$log_file);	
				}		
					
				$partDuration = time - $partTime;
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished\t$prog_used (flagstat_rmdup)\tsample\t$sample_id\t$partDuration\tseconds\n",$log_file);
				$partTime = time;			
			}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot start flagstat_rmdup: $bam_input is not present. Please check what is happen. \n",$log_file);
			}
	}
	...
	if ( ! ($flagstat_aln eq ''  and $flagstat_recalibrated eq '') ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Writing a table $reads_stats_tab with statistics from flagstat $flagstat_aln and $flagstat_recalibrated ... \n",$log_file);	
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: Both $flagstat_aln and $flagstat_recalibrated are empty. Please check what is happen. \n",$log_file);	
	}
}
...
sub run_cnv_finalout_steps{
	...
	#Get the parameters from input
	my $cfg_hash = load_hash($config_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting CNV output management and results writing..\n",$log_file);
	...
	#Build the output
	if ( $cfg_hash->{'cnv_out_build'} eq 'YES'){
		#############
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Moving the output from ExomeDepth for the different samples into the corresponding analysis folder..\n",$log_file);
		...
		#Operation 1:
		#In Exome Depth output, for each sample of the analysis, finds the output, adds the column with the sample name and concatenate all the files
		#Also merges SEX and AUTO results
		my $ed_out = $cnvoutFolder."/exomedepth.txt";
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Pre-processing ExomeDepth output for $analysis_name..\n",$log_file);	
		...
		if ( $cfg_hash->{'cnv_use_subject_only'} eq 'YES') {
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Pre-processing XHMM output for $analysis_name..\n",$log_file);		
			XHMM_output_preprocessing($cfg_hash,$analysis_id,$cnvoutFolder,$xhmm_out,$log_file);		
		}
		#Otherwise, only merge the results for auto and sex
		else{
			my $auto_o = $cnvoutFolder."/auto/".$cfg_hash->{'xhmm_xcnv_out'};
			my $sex_o = $cnvoutFolder."/sex/".$cfg_hash->{'xhmm_xcnv_out'};
			print_and_log( "Appending $auto_o and $sex_o to $xhmm_out\n",$log_file);#DEBUGCODE		
			append_file_2_file($auto_o,$xhmm_out);
			append_file_2_file($sex_o,$xhmm_out,"nohead");		
		}


		#Operation 3. Merge the output from XHMM and ExomeDepth
		#First add the compid column
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Adding compid column to both  $xhmm_out and $ed_out..\n",$log_file);			
		#..to XHMMM
		if ( count_lines_file($xhmm_out) > 1 ){
			my $tempout = $xhmm_out.".temp";
			my $RLogPath = $cnvoutFolder."/".$cfg_hash->{'log_fold'}."/$analysis_name\_CNV_GET_COMPID_xhmm".$cfg_hash->{'R_log_file'};
			my $R_script = $cfg_hash->{'lib_folder'}."/".$cfg_hash->{'R_utils'};	
			my $command = $cfg_hash->{'R_path'}." CMD BATCH --no-save --no-restore '--args CNV_GET_COMPID $xhmm_out xhmm $tempout' $R_script $RLogPath";
			
			print_and_log("The command is: $command\n",$log_file);
			try_exec_command($command) or R_die($command,$R_script);	
			
			move ($tempout,$xhmm_out) or die "Cannot move $tempout in $xhmm_out";	
		}else{print_and_log("$xhmm_out is empty. It will not be used.\n",$log_file);}
		#...to ExomeDepth
		if ( count_lines_file($ed_out) > 1 ){
			my $tempout = $ed_out.".temp";
			my $RLogPath = $cnvoutFolder."/".$cfg_hash->{'log_fold'}."/$analysis_name\_CNV_GET_COMPID_ed_out".$cfg_hash->{'R_log_file'};
			my $R_script = $cfg_hash->{'lib_folder'}."/".$cfg_hash->{'R_utils'};	
			my $command = $cfg_hash->{'R_path'}." CMD BATCH --no-save --no-restore '--args CNV_GET_COMPID $ed_out exomedepth $tempout' $R_script $RLogPath";
			
			print_and_log("The command is: $command\n",$log_file);
			try_exec_command($command) or R_die($command,$R_script);
			
			move ($tempout,$ed_out) or die "Cannot move $tempout in $ed_out";	
		}else{print_and_log("$ed_out is empty. It will not be used.\n",$log_file);}
		
		
		if ( count_lines_file($xhmm_out) > 1 and count_lines_file($ed_out) > 1 ){
			#If both files have CNVs, merge them using the compid		
			my $rfunction = "CNV_MERGE_OUTS";
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Merging output from XHMM and ExomeDepth for $analysis_name ($rfunction)..\n",$log_file);	
			my $RLogPath = $cnvoutFolder."/".$cfg_hash->{'log_fold'}."/$analysis_name\_$rfunction".$cfg_hash->{'R_log_file'};
			my $R_script = $cfg_hash->{'lib_folder'}."/".$cfg_hash->{'R_utils'};	
			my $command = $cfg_hash->{'R_path'}." CMD BATCH --no-save --no-restore '--args $rfunction $xhmm_out $ed_out $cnv_out' $R_script $RLogPath";
			
			print_and_log("The command is: $command\n",$log_file);
			try_exec_command($command) or R_die($command,$R_script);	
		}
		#Otherwise take one or the other as final output
		elsif ( count_lines_file($xhmm_out) > 1 ) {
			print_and_log("[an:$analysis_id ](".scalar(localtime).") The final output will be the one from XHMM for $analysis_name..\n",$log_file);	
			copy($xhmm_out,$cnv_out) or die "ERROR: Cannot copy $xhmm_out to $cnv_out \n";
		}elsif ( count_lines_file($ed_out) > 1 ) {
			print_and_log("[an:$analysis_id ](".scalar(localtime).") The final output will be the one from ExomeDepth for $analysis_name..\n",$log_file);			
			copy($ed_out,$cnv_out) or die "ERROR: Cannot copy $ed_out to $cnv_out \n";	
		}else{
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Merged output from XHMM and ExomeDepth is empty..\n",$log_file);		
		}
	}
	...
	if ( $cfg_hash->{'cnv_out_annote'} eq 'YES'){
		#To annotate using OMIM, for each interval we must find the gene associated.
		#To find it we can call the subroutine get_gene_4_interval but this requires in input a BED file
		#with "chr start end". This file can be created from the compid fields
		
		#Get the compid column
		my $only_compid_f = $cnv_out.".compid";
		my $compid_col = extract_col_from_file($cnv_out,'compid',$only_compid_f);
		
		#Get uniq compids
		my $command = " sort $only_compid_f | uniq > $only_compid_f.uniq";
		print_and_log( "\nExecuting command: $command\n",$log_file);#DEBUGCODE
		try_exec_command($command) or die "Unable to execute command: $command\n";
		
		#Separate compid to a BED file only with chr and interval
		#From chrx:283893-119393_DUP -> chrX\t283893\t119393
		compid_intervals_2_bed("$only_compid_f.uniq",$only_compid_f.".".$cfg_hash->{'bed_ext'});
		...
		my $cnv_geneann = "$only_compid_f\_gene.".$cfg_hash->{'bed_ext'};
		print_and_log( "...intersection with ucsc genes (intersectBed)...",$log_file);#DEBUGCODE
		run_BEDTOOLS_intersectBed($cfg_hash,$only_compid_f.".".$cfg_hash->{'bed_ext'},$ucsc_genes,$cnv_geneann,$inters_params,$log_file);
		#Leaving only: compid and gene
		my $cnv_gene_filt = $cnv_geneann.".filt";
		my $cols_to_extract = "3,7";
		print_and_log( "...getting only: (compid gene), and sort by compid...",$log_file);#DEBUGCODE
		extract_columns_from_file($cnv_geneann,$cols_to_extract,$cnv_gene_filt);
		$command = " sort -k1,1 $cnv_gene_filt > $cnv_gene_filt.sort";
		print_and_log( "\nExecuting command: $command\n",$log_file);#DEBUGCODE
		try_exec_command($command) or die "Unable to execute command: $command\n";	
		
		#move("$cnv_geneann.temp.sort",$cnv_geneann);	
		print_and_log( "...DONE!",$log_file);#DEBUGCODE	
		
		#Merge same compids with different genes (really big intervals!)
		#This:
		#chr10:101992985-102307894_DUP	CWF19L1,SNORA12,U6
		#chr10:101992985-102307894_DUP	BLOC1S2
		#Becomes:
		#chr10:101992985-102307894_DUP	CWF19L1,SNORA12,U6,BLOC1S2
		merge_rows_with_same_col("$cnv_gene_filt.sort","$cnv_gene_filt.sort.uniq","0,1");
		
		#Now add the header to the compid with genes
		print_and_log( "..add the header to the compid with genes...",$log_file);#DEBUGCODE
		insert_line_into_file("$cnv_gene_filt.sort.uniq","compid\tgene\n",0);
		
		#Finally merge the gene description with the annotation
		my $cnv_rearranged_out = "$cnvoutFolder/";
		if ( $cfg_hash->{'cnv_use_subject_only'} eq 'YES') {$cnv_rearranged_out .= "$analysis_name\_";}
		$cnv_rearranged_out .= "vargenius_cnv_".$cfg_hash->{'ann_build_ver'}.".".$cfg_hash->{'txt_ext'};
		print_and_log( "Merging the gene description with the annotation into $cnv_rearranged_out ...",$log_file);#DEBUGCODE
		rearrange_cnv_output($cfg_hash,$analysis_id,$cnv_out,$cnv_rearranged_out,"$cnv_gene_filt.sort.uniq",$log_file);	
	}
...
sub run_cnv_calling_pipeline{
	...
	#Get the parameters from input
	my $cfg_hash = load_hash($config_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting CNV pipeline..\n",$log_file);
	...
	#Run ExomeDepth
	my $edjobids = "";
	if ($cfg_hash->{'cnv_run_exomedepth'} eq 'YES'){
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Running ExomeDepth..\n",$log_file);
	...
	#Run XHMM
	my $xhmmjobids = "";
	if ($cfg_hash->{'cnv_run_xhmm'} eq 'YES'){	
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Running XHMM..\n",$log_file);
		
		$xhmmjobids = run_xhmm_pipeline($cfg_hash,$outFolder,$analysis_name,$analysis_id,\@steps_array2,$task,$log_file);
	}
	...
	my $new_dep_ids = "";
	if ( $edjobids ne ''){ $new_dep_ids .= ":$edjobids";}
	if ( $xhmmjobids ne ''){ $new_dep_ids .= ":$xhmmjobids";}
	my $job_name = $cfg_hash->{'cnv_finalout_step'}."_a".$analysis_id;
	my $cnvfout_jobid = JOB_get_jobid_form_jobname($job_name,$cfg_hash->{'qsub_username'});
	if ( $cnvfout_jobid > 0 ){ 
		print_and_log("Altering dependencies of ".$cfg_hash->{'cnv_finalout_step'}."_a".$analysis_id." ($cnvfout_jobid):\n ",$log_file);
		alter_job($cnvfout_jobid,$new_depend.$new_dep_ids,$log_file);
		
		#Update the hash with dependencies
		my $deps_f = $cfg_hash->{'main_data_folder'}."/".$cfg_hash->{'jc_dep_hash_f'};
		update_dependencies_hash($deps_f,$job_name,$cnvfout_jobid,$new_dep_ids,$log_file);
	}
}
...
sub run_coverage_per_analysis_steps{
	...
	#Get the parameters from input
	my $cfg_hash = load_hash($config_file);
	print_and_log("[an:$analysis_id ](".scalar(localtime).") Starting coverage study using GATK-DepthOfCoverage [per-analysis]..\n",$log_file);
	...
	foreach my $sample_id (keys %{$group_sam}){
		#print_and_log("Verifying sample $sample_id: ",$log_file);#DEBUGCODE
			...
			if ( -e $bam_input) {
				...
				die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot find file $bam_input that is needed for $task. Please run $refinetask task..\n";
			}
	}
	...
	open (PATHS,">".$bam_paths_f) or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot open $bam_paths_f. The program will exit..\n";
	...
	if ($cfg_hash->{$task_name} eq 'YES' ){
		if ( -e  $bam_paths_f){
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Starting the step to get $task_name : ",$log_file);
			run_GATK_DepthOfCoverage($cfg_hash,$bam_paths_f,$covgenes_suff,$analysis_id,$log_file);
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") The output suffix is: $covgenes_suff..\n",$log_file);
			$partDuration = time - $partTime;
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished $task_name $partDuration seconds\n",$log_file);
			$partTime = time;		
		}else{print_and_log( "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot get $task_name for analysis $analysis_name. File of paths $bam_paths_f does not exists.\n",$log_file);}
	}
	...
	foreach my $doc_out (@doc_outs){
		...
		if ( -e $out_f){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $out_f path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);	
			my $line = "-\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_cov_'.$doc_out}."\t".$out_f;
			append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line);
		}else{
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $out_f is not present. Please check what is happen. \n",$log_file);	
		}	
	}
...
#=head2 run_coverage_per_analysis_steps
 ...
 #Function: Coverage per analysis using DepthOfCoverage
						...
						#This subroutine launches DepthOfCoverage for the BAM file after alignment 
						#and mark duplicates
...
#sub run_coverage_per_analysis_steps_ALN{
	...
	##and it is executed during the stats step. Hence it will block you and 
	##ask to run that before
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") Checking if there are multiple read file for each sample. Because in that case".
	...
	#foreach my $sample_id (keys %{$group_sam}){
		...
		##If there are multiple read files than it should have been run the step for merge
			...
			##print_and_log("[an:$analysis_id ](".scalar(localtime).") yes! -> samplename: $sample_name. Using input file: $bed_merged\n",$log_file);#DEBUGCODE
			#if ( -e $bed_merged) {
				#push(@paths_to_write,$bed_merged);
			#}else{
				#die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot find file $bed_merged that is needed for $task. Please run stats step..\n";
			#}
...
sub run_fast_final_out_steps{
	...
	##March 2019 - Since I am not using this lock for the fastfout. I have commented this snip of code
	#print_and_log("[an:$analysis_id ](".scalar(localtime).") Setting analysis_id $analysis_id to be the last waiter of the db for the final output ...\n",$log_file);
	...
	#Getting the sample name
	my $db_analysis_name = $cfg_hash->{'db_analysis_name'};
	...
	my $vcf_file = $inFolder."/".build_input_name_from_executed($params,$step,
								$params->{$db_analysis_name},\@steps_array3).".".$cfg_hash->{'vcf_ext'};
	...
	if ( $consensus) {
		...
		#Search if there are VCF files apart of GATK
		...
		if ( $cfg_hash->{'vc_output_paths'} ne ''){
			...
			my $vc_alg_num = 0;
			foreach my $vc_algorithm (@vc_algorithms){
				vcf_insert_string_into_field($vc_output_paths[$vc_alg_num],"VC_ALGORITHM=".$vc_algorithms[$vc_alg_num],uc($cfg_hash->{'db_vcf_info_str'}),$vc_output_paths[$vc_alg_num].".algostr");
				$vc_alg_num++;
			}
			...
			if ($cfg_hash->{'consensus_vc_approach'} eq 'UNION'){
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).") The ".$cfg_hash->{'consensus_vc_approach'}." of VCF outputs will be performed and $final_vcf generated..\n",$log_file);	
				
				#Join the VCF files generated from all the variant calling software
				my $vc_alg_num = 0;
				my $first_vcf = $vcf_file;
				foreach my $vc_algorithm (@vc_algorithms){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Joining $first_vcf and ".$vc_output_paths[$vc_alg_num].".temp into $final_vcf.temp..\n",$log_file);			
					join_vcf_files_with_cat($first_vcf,$vc_output_paths[$vc_alg_num].".algostr",$final_vcf.".temp");
				...
				#genotype)
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Sorting $final_vcf.temp into $final_vcf.sort ..\n",$log_file);			
				run_VCFTOOLS_sort($cfg_hash,$first_vcf,$final_vcf.".sort","",$log_file);
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing identical calls from $final_vcf.sort and saving into $final_vcf..\n",$log_file);
				vcf_filter_identical_calls($cfg_hash,$final_vcf.".sort",$final_vcf,$log_file);
				
				$vcf_file = $final_vcf;

				#Delete all temporary files
				if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the following temporary files: $final_vcf.temp, $final_vcf.sort..\n",$log_file);	
					delete_file($final_vcf.".temp");
					delete_file($final_vcf.".sort");
				}		
			...
			##Addition of Unique variants to GATK result
				...
				foreach my $vc_algorithm (@vc_algorithms){
					...
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Get the variants from $current_vcf that are not in GATK $vcf_file..\n",$log_file);	
					...
					#Join the two VCF files
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Joining $first_vcf and $vcf_uniq into $final_vcf.temp..\n",$log_file);			
					...
					#Delete all temporary files
					if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
						print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the following temporary files: $vcf_uniq..\n",$log_file);	
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Sorting $final_vcf.temp into $final_vcf..\n",$log_file);			
				run_VCFTOOLS_sort($cfg_hash,$final_vcf.".temp",$final_vcf,"",$log_file);			
				$vcf_file = $final_vcf;

				#Delete all temporary files
				if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing the following temporary files: $final_vcf.temp..\n",$log_file);	
					delete_file($final_vcf.".temp");
				}						
			...
			#This is the real consensus approach because choses all the calls only that are called from all the callers
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).") No consensus approach has been used (".$cfg_hash->{'consensus_vc_approach'}."). The result will be only from GATK.\n",$log_file);
			}
  ...
  if ($genotref == 1){
		if ($cfg_hash->{'run_samples'} ne 'ALL'){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Since the VQSR has been executed we use a VCF with only selected samples:\n ".
			...
			if ( -e $vcf_file.".all"){
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Moving $vcf_file to $vcf_file.all \n",$log_file);			
				move($vcf_file, $vcf_file.".all") or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot move $vcf_file in $vcf_file.all\n";
			}
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Moving $temp_out in $vcf_file \n",$log_file);	
			move($temp_out, $vcf_file) or die "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot move $temp_out in $vcf_file\n";      
		}
	...
	#################END VQSR Considerations	
		...
		if ( -e $vcf_file){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Executing create_vcf_to_ann. Two tables will be generated from $vcf_file".
			...
			$partTime = time;	
				...
				log_and_exit("[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot start $prog_used for analysis $sample_id. Missing the VCF output: $vcf_file\n",$log_file);
		}
	}
	...
	if ($cfg_hash->{'annov_db_dl'} eq 'YES'){
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Checking the existence of Annovar database...\n",$log_file);
		...
		foreach my $ann_type (@ann_types){
			if ( defined $cfg_hash->{'annov_'.$ann_type.'_dbs'}) {
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Installing ".scalar(@databases)." databases for Annovar ($ann_type) using $prog_used \n",$log_file);
				print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used..\n",$log_file);	
				#Download each database if it does not exists yet
				foreach my $db_2_dl (@databases){
						print_and_log( "[an:$analysis_id ](".scalar(localtime).") Checking database: $db_2_dl\n",$log_file);#DEBUGCODE
						...
						my $db_present = "";
						#print_and_log( "[an:$analysis_id ](".scalar(localtime).") Searching $db_2_dl and $annovar_ver into database: $db_2_dl\n",$log_file);#DEBUGCODE
						...
						if ( length $db_present <= 2 ){
							...
							if( $db_2_dl !~ /\_EXT$/  and $db_2_dl !~ /\_BED$/){							
								...
								if( $db_2_dl =~ /Gencode/ ){
									...
									print_and_log( "[an:$analysis_id ](".scalar(localtime).") Installing database $db_2_dl for Annovar : ",$log_file);
								...
								print_and_log( "[an:$analysis_id ](".scalar(localtime).") Database $db_2_dl is EXT or BED \n",$log_file);#DEBUGCODE
								my $path = "";
								my $db_name = $db_2_dl;
								if ($db_name =~ /\_EXT$/){
								...
								if ( ! -e  $path ){
									log_and_exit( "[an:$analysis_id ](".scalar(localtime).") WARNING: Database $path for Annovar does not exist in ".$cfg_hash->{'annovar_db_f'}.
							...
							print_and_log("[an:$analysis_id ](".scalar(localtime).") Updating table with database $db_2_dl in ".$cfg_hash-> {'db_name'}." ...\n",$log_file);
							insert_into_table_locked($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},
									$cfg_hash->{'db_user'},$cfg_hash->{'db_pass'},$cfg_hash->{'db_annovar_dbs_table'},
									$fields,$values);
						}else{
							print_and_log( "[an:$analysis_id ](".scalar(localtime).") Database $db_2_dl for Annovar already exists... \n",$log_file);
						}	
				}#code
			}	
		}
	...
	my $allvar_out_suff = $outFolder."/".$params->{$db_analysis_name}."_".$cfg_hash->{'allvar_ext'};
	if ($cfg_hash->{'annot_all'} eq 'YES'){

		my $prog_used = $cfg_hash->{'table_annov_prog'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Annotate all variants in VCF file $vcf_all_var ...\n",$log_file);
		print_and_log("[an:$analysis_id ](".scalar(localtime).")  RUNNING $prog_used : ",$log_file);	
		...
		if (file_num_rows($vcf_all_var) > 1) {
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Convert VCF format file $vcf_all_var to ANNOVAR format in $in_annov : ",$log_file);									
			run_ANNOVAR_convert2annovar($cfg_hash,$prog_used ,$log_file,$vcf_all_var,$in_annov);
			#Annotate all with all the databases and settings from the user configuration
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Annotating $in_annov : ",$log_file);			
			run_ANNOVAR_table_annovar($cfg_hash,$prog_used,$log_file,$in_annov,$allvar_out_suff);
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Completed the annotation of $in_annov. Output file is: $allvar_out_suff...\n",$log_file);
			
			#Delete the input for Annovar
			if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing  $in_annov..\n",$log_file);	
			...
			log_and_exit("[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot annotate  $in_annov. File empty ...\n",$log_file);
		}	
	...
	if ($cfg_hash->{'write_output'} eq 'YES'){
		...
		if ( -e $compl_annovar_out and !(-z $compl_annovar_out) ){
			if ( -e $vcf_file  and   !(-z $vcf_file) ){
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Extracting the final output using the annotation ($compl_annovar_out), $vcf_all_var and $vcf_all_var.info into ($vargenius_out) ..\n",$log_file);
				print_and_log("[an:$analysis_id ](".scalar(localtime).")  RUNNING $prog_used : ",$log_file);	
				...
				foreach my $func_col (@func_cols){
					...
					#print_and_log("[an:$analysis_id ](".scalar(localtime).") Inverting columns $func_col_ind and ".($func_col_ind+1)." to shift $func_col ",$log_file);#DEBUGCODE
				...
				########################New shift of the internal frequency after the other frequencies
				#March 2019
				my $tempout = $vargenius_out.".temp";
				my $RLogPath = $cfg_hash->{$analysis_id.'_'.$task.'_out_f'}."/".$cfg_hash->{'log_fold'}."/$analysis_name\_SHIFT_COL".$cfg_hash->{'R_log_file'};
				my $R_script = $cfg_hash->{'lib_folder'}."/".$cfg_hash->{'R_utils'};	
				my $command = $cfg_hash->{'R_path'}." CMD BATCH --no-save --no-restore '--args SHIFT_COL $vargenius_out $tempout ".$cfg_hash->{'intern_var_freq_fld'}." ".$cfg_hash->{'last_1000g_field'}."' $R_script $RLogPath";
				
				print_and_log("The command is: $command\n",$log_file);
				try_exec_command($command) or R_die($command,$R_script);
				my $tempout2 = $vargenius_out.".temp2";	
				$command = $cfg_hash->{'R_path'}." CMD BATCH --no-save --no-restore '--args SHIFT_COL $tempout $tempout2 ".$cfg_hash->{'var_frequency_factors_fld'}." ".$cfg_hash->{'intern_var_freq_fld'}."' $R_script $RLogPath";
				
				print_and_log("The command is: $command\n",$log_file);
				try_exec_command($command) or R_die($command,$R_script);
								
				move ($tempout2,$vargenius_out) or die "Cannot move $tempout2 in $vargenius_out";	
				#Delete the temporary file remained
				if ( $cfg_hash->{'remove_temp'} eq 'YES' ){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Removing $tempout...\n",$log_file);	
					delete_file($tempout);
				}	
				########################
				...
				log_and_exit("[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot write the output $vcf_file is missing or empty ...\n",$log_file);
			}	
		}else{
			log_and_exit("[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot write the output $compl_annovar_out is missing or empty..\n",$log_file);
		}	
	...
	my $rearr_ann_out =  $outFolder."/".$params->{$db_analysis_name}.".".$cfg_hash->{'ann_build_ver'}.".final_out_resgr$resgroupid.txt";
	my $rearr_ann_out_xls =  $outFolder."/".$params->{$db_analysis_name}.".".$cfg_hash->{'ann_build_ver'}.".final_out_resgr$resgroupid.xlsx";
	if ($cfg_hash->{'rearrange_output'} eq 'YES'){
		...
		if ( -e $output_2_rearrange and !(-z $output_2_rearrange)) {
			if ( correct_type($resgroupid,"positiveint")){							
				...
				print_and_log("[an:$analysis_id ](".scalar(localtime).")  RUNNING $prog_used rearrange_output..\n",$log_file);		
				print_and_log("[an:$analysis_id ](".scalar(localtime).") Rearranging the output in $output_2_rearrange making $rearr_ann_out...\n",$log_file);
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Converting the TSV file ($rearr_ann_out) to XLS format ($rearr_ann_out_xls)...\n",$log_file);
				...
				log_and_exit( "[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot execute rearrange_output for research group: $resgroupid\n",$log_file);
			}
		}else{
			log_and_exit("[an:$analysis_id ](".scalar(localtime).") WARNING: Cannot find $output_2_rearrange. $prog_used will not be executed...\n",$log_file);
		}
	}
	...
	if ($cfg_hash->{'sep_joint_analyses'} eq 'YES'){
		print_and_log("[an:$analysis_id ](".scalar(localtime).")   RUNNING sep_joint_analyses..\n",$log_file);	
		...
		foreach my $out_path (@out_paths){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Converting the TSV file ($out_path) to XLS format ($out_path_xls)...\n",$log_file);
			tsv_2_xls($out_path,$out_path_xls);				

			#Put the path to the Final tabular output in the outlist file
			if ( check_presence($out_path) ){
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $out_path path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
				my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_out'}." ".extract_name($out_path,0)."\t".$out_path;
				append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
			}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $rearr_ann_out is not present. Please check what is happen. \n",$log_file);	
			}		
			
			#Put the path to the XLS version of the Final tabular output in the outlist file
			if ( check_presence($out_path_xls) ){
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $out_path_xls path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
				my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_xls'}." ".extract_name($out_path_xls,0)."\t".$out_path_xls;
				append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
			}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $rearr_ann_out_xls is not present. Please check what is happen. \n",$log_file);	
			}	
		}
	}	
	...
	if ($cfg_hash->{'separate_analysis_from_joint'} eq 'YES'){
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).")  RUNNING separate_analysis_from_joint..\n",$log_file);	
		...
		my @samples_ids = split($sep,$samples_ids);
		my $sample_names = "";
		#print_and_log( "[an:$analysis_id ](".scalar(localtime).") Picking informations only for $sample_names..\n",$log_file);#DEBUGCODE
		foreach my $sel_sample_id (@samples_ids){
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Writing information only for $sample_names into file $out_path...\n",$log_file);
		...
		if ( check_presence($out_path) ){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Filtering variants for $sample_names (remove GT=0/0 or 0|0 for all samples)...\n",$log_file);
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") The command is: $command\n",$log_file);
			try_exec_command($command) or R_die($command,$R_script);			
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $out_path is not present. Please check what is happen. \n",$log_file);	
		}
		...
		if ( check_presence($out_path_xls) ){
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $out_path_xls is not present. Please check what is happen. \n",$log_file);	
		}	
	...
	if ($cfg_hash->{'supplementary_output'} eq 'YES'){
		...
		if ( -e $rearr_ann_out and !(-z $rearr_ann_out) ){
			...
			#print_and_log("[an:$analysis_id ](".scalar(localtime).") Samples header: " ,$log_file);#DEBUGCODE
			...
			#Convert the tsv file to xls in the same location
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Converting the TSV file ($out2nd_lev_fin) to XLS format ($out2nd_lev_fin_xls)...\n",$log_file);
			...
			if ($ped_file ne 'none'){				
				...
				unless(-d $priorwdir){
					print_and_log("[an:$analysis_id ](".scalar(localtime).") Creating folder $priorwdir...\n",$log_file);
					mkdir $priorwdir or die "[an:$analysis_id ](".scalar(localtime).") ERROR: can't create folder $priorwdir. Check permissions. \n";
				}
				`chmod 777 $priorwdir`;
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") ERROR: cannot start $prog_used. File $rearr_ann_out does not exists... \n",$log_file);
		}		
	}
	...
	if ($cfg_hash->{'get_coverage_plots'} eq 'YES'){	
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting the samples list associated with the analysisid... \n",$log_file);
		my $samples_h;#Samples hash to be reordered
		my @sort_samples = ();#The array with samples id reordered
		#Get the samples names involved for the group
		my $query = "SELECT ".$cfg_hash->{'db_sample_name'}." FROM  ".$cfg_hash->{'db_sample_table'}." WHERE ".$cfg_hash->{'db_analysis_id'}."=$analysis_id;";	
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting the gene coverage above $percentage_above... \n",$log_file);
		...
		if ( -e $out_genecov_above){
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting plots of coverage per-disease... \n",$log_file);
			...
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. File $out_genecov_above does not exists... \n",$log_file);
		}	
		...
		if ( -e $out_genecov_above){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting tables for all genes and all disease with coverage less than 99%... \n",$log_file);
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") The command is:\n $command\n",$log_file);
			try_exec_command($command) or R_die($command,$R_script);
			...
			foreach my $sample_name (keys %{$group_sam}){  
				...
				if ( check_presence($outsuffix."_".$sample_name) ){
					print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $outsuffix\_$sample_name path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
					my $line = "$sample_name\t-\t".$cfg_hash->{'outlist_cov_page'}."\t".$cfg_hash->{'outlist_cov_'.$plot_name}."\t".$outsuffix."_".$sample_name;
					append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
				}else{
					print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $outsuffix\_$sample_name is not present. Please check what is happen. \n",$log_file);	
				}	
			}
		}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. File $out_genecov_above does not exists... \n",$log_file);
		...
		if ( -e $rearr_ann_out and !(-z $rearr_ann_out)){
			...
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting High Quality Variants Plots (Dispersion) : ",$log_file);	
			my $RLogPath = $statsFolder."/log/$analysis_name\_$plot_name".$cfg_hash->{'R_log_file'};
			...
			try_exec_command($command) or R_die($command,$R_script);
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") ERROR: cannot start $plot_name. File $rearr_ann_out does not exists... \n",$log_file);
		}	
	}
	...
	if ($cfg_hash->{'gene_annotation'} eq 'YES'){			
		if ( -e $rearr_ann_out and !(-z $rearr_ann_out)){
			my $prog_used = "VarGenius";	
			print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING $prog_used gene_annotation..\n",$log_file);	

			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Executing gene annotation of $rearr_ann_out...\n",$log_file);
			...
			$partDuration = time - $partTime;
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") Finished $prog_used-gene_annotation $partDuration seconds\n",$log_file);
			$partTime = time;					
		}else{
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start gene_annotation. File $rearr_ann_out does not exists... \n",$log_file);
		}	
	...
	if ( $cfg_hash->{'do_users_modifies'} eq 'YES' ){
			...
			if ( (defined $cfg_hash->{'panel_to_tag'}) and length($cfg_hash->{'panel_to_tag'}) > 2 ){
					...
					print_and_log( "[an:$analysis_id ](".scalar(localtime).") Generating an additional output (XLSX) adding a flag for each $panel gene...\n",$log_file);
	...
	if ( check_presence($vcf_file) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $vcf_file path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_vcf'}."\t".$vcf_file;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $vcf_file is not present. Please check what is happen. \n",$log_file);	
	}		

	#Put the path to the VCF index file in the outlist file
	my $vcfi_file = extract_name($vcf_file,"noext").".".$cfg_hash->{'vcfi_ext'};
	if ( check_presence($vcfi_file) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $vcfi_file path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_vcfindex'}."\t".$vcfi_file;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $vcfi_file is not present. Please check what is happen. \n",$log_file);	
	}
	
	#Put the path to the Final tabular output in the outlist file
	if ( check_presence($rearr_ann_out) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $rearr_ann_out path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
	...
	if ( check_presence($rearr_ann_out_xls) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $rearr_ann_out_xls path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
	...
	if ( check_presence($out2nd_lev) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $out2nd_lev path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_2ndlev'}."\t".$out2nd_lev;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );		
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $out2nd_lev is not present. Please check what is happen. \n",$log_file);	
	}

	#Put the gene annotation into the outlist file if present
	if ( check_presence($gene_annotation_out) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $gene_annotation_out path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_final_geneann'}."\t".$gene_annotation_out;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );		
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $gene_annotation_out is not present. Please check what is happen. \n",$log_file);	
	}


	#Put the prioritization directory into the outlist file if present
	if ( -d $priorwdir and !(is_folder_empty($priorwdir)) ){
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $priorwdir path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_prioritization_fold'}."\t".$priorwdir;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );		
	}else{
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: $priorwdir is not present or empty. Please check what is happen. \n",$log_file);	
	}
	...
	if ($cfg_hash->{'update_overall_samples_plots'} eq 'YES'){
		print_and_log("[an:$analysis_id ](".scalar(localtime).") RUNNING  update_overall_samples_plots..\n",$log_file);	
		#########################
		#Set and get needed data
		#########################
		#Some statistics will be take from the stats folder
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting all samples coverage plot (percentage of locus)... \n",$log_file);	
		...
		if ( -e $all_sam_cov_tab and !(-z $all_sam_cov_tab)){
			...
			if ( $success1 > 0){
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. Bad result from get_all_samples_coverage_table... \n",$log_file);
			}

		}else {
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. Empty result from get_all_samples_coverage_table... \n",$log_file);
		}		
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting all samples coverage plot (number of locus)... \n",$log_file);	
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Getting all samples reads number plot ($plot_name) ... \n",$log_file);
		...
		if ( -e $all_sam_reads_tab and !(-z $all_sam_reads_tab)){
			...
			if ( $success2 > 0){
				...
				print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. Bad result from get_all_samples_reads_number... \n",$log_file);
			}
			#delete_file($all_sam_reads_tab_temp);
		}else {
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") WARNING: cannot start $plot_name. Empty result from get_all_samples_reads_number... \n",$log_file);
		}				
	}		
	...
	if ($cfg_hash->{'generate_html'} eq 'YES'){			
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Executing HTML productions of $rearr_ann_out...\n",$log_file);
		...
		if ( !-e $outlist_f ) {log_and_exit("[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot proceed with generate_html. $outlist_f missing.\n",$log_file);}
		if ( !-e $rearr_ann_out) {log_and_exit("[an:$analysis_id ](".scalar(localtime).") ERROR: Cannot proceed with generate_html. $rearr_ann_out missing.\n",$log_file);}

		print_and_log("[an:$analysis_id ](".scalar(localtime).") Reading $outlist_f to generate an hash...\n",$log_file);
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Printing statistics about the results obtained in $stats_out...\n",$log_file);
		print_variants_stats($cfg_hash,$analysis_id,$rearr_ann_out,$task,$log_file,$stats_out);
		#Update table with statistics from this file written with the keyword #DB
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Loading statistics into the database...\n",$log_file);
		...
		my $xml_igv_session_out = "$outFolder/$analysis_name\_igv_session.".$cfg_hash->{'txt_ext'};
		print_and_log("[an:$analysis_id ](".scalar(localtime).") Generating the IGV session file in $xml_igv_session_out...\n",$log_file);
		generate_igvsession_xml($cfg_hash,$paths_hash,$xml_igv_session_out,$log_file);
		##I don't know why but I have to use  a temp file...
		#my $new = extract_name($xml_igv_session_out,"noext").".".$cfg_hash->{'xml_ext'};
		#copy ($xml_igv_session_out,$new) or die "ERROR: Unable to copy $xml_igv_session_out to $new\n";
		#$xml_igv_session_out = $new;
		#Add the IGV session file to the paths hash
		$paths_hash->{ $cfg_hash->{"outlist_output_page"} }->{'-'}->{'-'}->{ $cfg_hash->{'outlist_igvsession_xml'} }->{'path'} = $xml_igv_session_out;
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $xml_igv_session_out path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		my $line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_igvsession_xml'}."\t".$xml_igv_session_out;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		
		#################################
		##HTML WEB PAGES
		#################################
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Generating the HTML website in $outFolder...\n",$log_file);

		################################
		#DATA COMPRESSION
		#Compress data and add path to the paths hash (first page)
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Compressing data to store into $outFolder...\n",$log_file);
		...
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $compressed_data_path path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		$line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_storage_compressed'}."\t".$compressed_data_path;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") STATISTICS DURING THE ANALYSIS:\n",$log_file);
		if ( -e $cfg_hash->{$analysis_id."_outstats_file"}){
			print_and_log("[an:$analysis_id ](".scalar(localtime).") Printing ".$cfg_hash->{$analysis_id."_outstats_file"}." to HTML...\n",$log_file);
			$html_string .= file_2_html($cfg_hash->{$analysis_id."_outstats_file"});
		}else{print_and_log("[an:$analysis_id ](".scalar(localtime).") ".$cfg_hash->{$analysis_id."_outstats_file"}." file does not exist...\n",$log_file);}
		...
		print_and_log("[an:$analysis_id ](".scalar(localtime).") STATISTICS FROM TABLES:\n",$log_file);

		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Extract and print statistics from tables (from $tables_list_file)...\n",$log_file);
		#print_and_log( "[an:$analysis_id ](".scalar(localtime).") The files will be printed into $outFolder...\n",$log_file);#DEBUGCODE
		...
		if (-e $tables_list_file ) {
			print_and_log( "[an:$analysis_id ](".scalar(localtime).") $tables_list_file exists. Using table2html...\n",$log_file);#DEBUGCODE
			$html_string .= table_list_2_html($tables_list_file,$cfg_hash->{'remove_temp'});
		}else{print_and_log("[an:$analysis_id ](".scalar(localtime).") WARNING: $tables_list_file has not been found..\n",$log_file);}
		...
		#All the HTML files must be included into the outlist file to be saved in the storage area
		#Img folder
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Appending $imgFolder path to ".$cfg_hash->{$analysis_id."_outlist_file"}." file... \n",$log_file);
		$line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t".$cfg_hash->{'outlist_html_imgf'}."\t".$imgFolder;
		append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );
		#HTML pages
		my @html_pages_suffs = split (",",$cfg_hash->{'outlist_html_pages'});
		foreach my $html_pages_s (@html_pages_suffs){
			my $path = "$outFolder/".$cfg_hash->{"html_$html_pages_s\_page"};
			my $desc = $cfg_hash->{"outlist_html_$html_pages_s"};
			$line = "-\t-\t".$cfg_hash->{'outlist_output_page'}."\t$desc\t".$path;
			append_str_2_file_if_path_notexist($cfg_hash->{$analysis_id."_outlist_file"},$line );			
		}
	...
	if ($cfg_hash->{'send_email'} eq 'YES'){			
		my $prog_used = "VarGenius";	
		print_and_log("[an:$analysis_id ](".scalar(localtime).")   RUNNING $prog_used send_email..\n",$log_file);	
				
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Sending an email to ".$cfg_hash->{'email_recipients'}."...\n",$log_file);
	...
	print_and_log( "[an:$analysis_id ](".scalar(localtime).") Updating the database with analysis information... \n",$log_file);
	...
	#Update the analysis table with the final status.
	...
	###################################################
	### SAVE THE ANALYSIS (COPY) INTO THE STORAGE PROTECTED AREA
	###################################################	
	#I'm using forcerun=1 
	#Send email when the analysis is completed
	if ($cfg_hash->{'save_analysis'} eq 'YES'){	
		print_and_log( "[an:$analysis_id ](".scalar(localtime).") Saving the analysis in the storage folder: ".$cfg_hash->{'storage_f'}."... \n",$log_file);		
		save_analyses($cfg_hash,$cfg_hash->{'work_fold'},$analysis_id,1,$log_file);
	}
	...
	#my $analysisin = get_id_if_exists_from_db($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
		...
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Freeing the db (db_busy=0) since $analysis_id is the last using it ...\n",$log_file);
		#update_table($cfg_hash->{'db_name'},$cfg_hash->{'db_dsn'},$cfg_hash->{'db_user'},
				#$cfg_hash->{'db_pass'},$cfg_hash->{'db_info_table'},$cfg_hash->{'db_busy'},"0",$cfg_hash->{'db_info_row'},"1");	
	#}else{
		#print_and_log("[an:$analysis_id ](".scalar(localtime).") Analysis $analysisin will use the database. This analysis ($analysis_id) does not have to free it up with db_busy=0...\n",$log_file);
	#}	
	...
	print_and_log("\n\n[an:$analysis_id ](".scalar(localtime)."):  VarGenius execution is completed for analysis $analysis_id...\n",$log_file);
}
...
sub run_import_2_db_steps{
	...
	#Input and output folders
	my $inFolder = $cfg_hash->{$analysis_id.'_'.$previous_task.'_out_f'};
	my $outFolder = $cfg_hash->{$analysis_id.'_'.$fold_suff.'_out_f'};
