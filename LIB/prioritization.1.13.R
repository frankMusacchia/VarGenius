...
query.panelApp<-function(genename) {
  if (nchar(genename)<1) return()
  querytext=paste0("https://bioinfo.extge.co.uk/crowdsourcing/WebServices/search_genes/",genename,"/")
  ...
  for (l in 1:nrow(res$results)) {
    ...
    cat(res$results[l,7])
    cat("\nPhenotype ")
    cat(unlist(res$results[l,10]))
    cat("\n")
    cat(paste(res$results[l,c(1,6)],collapse=" // "))
    cat("\n")
  }
}
...
# Nomenclatura HGVS
outxt$HGVS<-apply(outxt,1,function(x) make.HGVS(x,cchrom,cpos,cref,calt))#FRANK
